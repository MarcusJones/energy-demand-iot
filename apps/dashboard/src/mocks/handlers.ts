...
export const handlers = [
  ...
  http.get("/api/sites", async ({ request }) => {
    ...
    const result = await repo.list({
      ...
      status: (params.get("status") as "active" | "inactive" | "commissioning") ?? undefined,
    });
    return HttpResponse.json(result);
  }),

  http.get("/api/sites/:id", async ({ params }) => {
    const repo = getMockSiteRepository();
    const site = await repo.getById(params.id as string);
    if (!site) return new HttpResponse(null, { status: 404 });
    return HttpResponse.json(site);
  }),

  http.get("/api/sites/:id/summary", async ({ params }) => {
    const repo = getMockSiteRepository();
    const summary = await repo.getSummary(params.id as string);
  ...
  http.get("/api/devices/:id", async ({ params }) => {
    const repo = getMockDeviceRepository();
    const device = await repo.getById(params.id as string);
    if (!device) return new HttpResponse(null, { status: 404 });
    return HttpResponse.json(device);
  }),

  http.get("/api/devices/counts/status", async ({ request }) => {
    const params = searchParams(request.url);
    const repo = getMockDeviceRepository();
  ...
  http.get("/api/devices/counts/type", async ({ request }) => {
    const params = searchParams(request.url);
    const repo = getMockDeviceRepository();
  ...
  // ── Readings ─────────────────────────────────────────
  http.get("/api/readings/:deviceId/latest", async ({ params }) => {
    const repo = getMockReadingRepository();
    const reading = await repo.getLatest(params.deviceId as string);
    if (!reading) return new HttpResponse(null, { status: 404 });
    return HttpResponse.json(reading);
  }),

  http.get("/api/readings/:deviceId/range", async ({ params, request }) => {
    const sp = searchParams(request.url);
    const repo = getMockReadingRepository();
  ...
  http.get("/api/readings/aggregate/:siteId", async ({ params, request }) => {
    const sp = searchParams(request.url);
    const repo = getMockReadingRepository();
  ...
  http.get("/api/readings/daily/:siteId", async ({ params, request }) => {
    const sp = searchParams(request.url);
    const repo = getMockReadingRepository();
  ...
  // ── Forecasts ────────────────────────────────────────
  http.get("/api/forecasts/:siteId/latest", async ({ params, request }) => {
    const sp = searchParams(request.url);
  ...
  http.get("/api/forecasts/:siteId/horizon", async ({ params, request }) => {
    const sp = searchParams(request.url);
    const repo = getMockForecastRepository();
  ...
  http.post("/api/schedules", async ({ request }) => {
    ...
    const repo = getMockScheduleRepository();
    const schedule = await repo.create(body as Parameters<typeof repo.create>[0]);
    return HttpResponse.json(schedule, { status: 201 });
  }),

  http.post("/api/schedules/:id/cancel", async ({ params }) => {
    const repo = getMockScheduleRepository();
    const schedule = await repo.cancel(params.id as string);
    if (!schedule) return new HttpResponse(null, { status: 404 });
    return HttpResponse.json(schedule);
  }),

  // ── Tariffs ──────────────────────────────────────────
  http.get("/api/tariffs/:siteId", async ({ params }) => {
    const repo = getMockTariffRepository();
    const tariffs = await repo.getForSite(params.siteId as string);
    return HttpResponse.json(tariffs);
  }),

  http.get("/api/tariffs/:siteId/active", async ({ params }) => {
    const repo = getMockTariffRepository();
    const tariff = await repo.getActive(params.siteId as string);
    if (!tariff) return new HttpResponse(null, { status: 404 });
    return HttpResponse.json(tariff);
  }),

  http.get("/api/tariffs/:siteId/price", async ({ params }) => {
    const repo = getMockTariffRepository();
    const price = await repo.getCurrentPrice(params.siteId as string);
    if (!price) return new HttpResponse(null, { status: 404 });
    return HttpResponse.json(price);
  }),

  // ── DR Events ────────────────────────────────────────
  http.get("/api/dr-events", async ({ request }) => {
    const params = searchParams(request.url);
  ...
  http.get("/api/dr-events/:id", async ({ params }) => {
    const repo = getMockDREventRepository();
    const event = await repo.getById(params.id as string);
